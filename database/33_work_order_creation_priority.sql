-- Lets the creator select a work order's priority instead of always using medium.

drop function if exists public.create_and_offer_work_order(text, text, text, text, text, text, timestamptz, uuid);
