...
export function buildMessage(event:SmsEvent,wo:string,actor:string,completedAt:string|null){
  ...
  if(event==='accepted'||event==='rejected') return `WO#${wo} has been ${event==='accepted'?'Accepted':'Rejected'} by ${actor}`;
  if(event==='completed'){const d=new Date(completedAt??Date.now());return `WO#${wo} has been Completed by ${actor} on ${d.toLocaleDateString('en-US',{weekday:'long',timeZone:'America/New_York'})} ${d.toLocaleDateString('en-US',{timeZone:'America/New_York'})} at ${d.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',timeZone:'America/New_York'})}`;}
  ...
  return `Work Order WO#${wo} has been modified by ${actor}: ${changes[event]}.`;
}
