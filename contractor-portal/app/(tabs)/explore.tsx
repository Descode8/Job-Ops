...
export default function ServicesScreen() {
  ...
  const displayedJobs = isAdmin && showCompleted ? jobs : activeJobs;
  const visibleJobs = useMemo(() => displayedJobs.filter((job) => filter === 'parts'
    ? needsParts(job)
    : filter === 'urgent' ? isUrgent(job) : true), [displayedJobs, filter]);
  const homeServices = visibleJobs.filter((job) => job.work_order_number.startsWith('HOME-'));
  const standardWorkOrders = visibleJobs.filter((job) => !job.work_order_number.startsWith('HOME-'));
  const contractorWorkOrders = isAdmin ? visibleJobs : standardWorkOrders;
  const myWorkOrders = standardWorkOrders.filter((job) => job.assignee?.id === currentContractorId);
  const otherWorkOrders = standardWorkOrders.filter((job) => job.assignee?.id !== currentContractorId);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.resultCount}>{isLoading ? 'LOADING...' : `${contractorWorkOrders.length} ${filter.toUpperCase()} WORK ORDER${contractorWorkOrders.length === 1 ? '' : 'S'}`}</Text>
        {isAdmin ? <>
          <ServiceGroup title="Home Progress" itemLabel="HOME" count={homeServices.length} expanded={homeServicesExpanded} onPress={() => setHomeServicesExpanded((value) => !value)}>{homeServices.length ? homeServices.map(renderJob) : <GroupEmpty message="No matching Home Progress records." />}</ServiceGroup>
          <ServiceGroup title="Work Orders" count={myWorkOrders.length} expanded={myWorkExpanded} onPress={() => setMyWorkExpanded((value) => !value)}>{myWorkOrders.length ? myWorkOrders.map(renderJob) : <GroupEmpty message="No matching work orders assigned to you." />}</ServiceGroup>
...
const createStyles = (colors: AppThemeColors) => StyleSheet.create({
  ...
  summary: { flex: 1, alignItems: 'center', paddingVertical: 15, borderRightWidth: 1, borderRightColor: colors.border },
  summaryValue: { color: YELLOW, fontSize: 20, fontWeight: '900' },
  summaryLabel: { color: PAPER, fontSize: 8, fontWeight: '900', marginTop: 4 },
  filters: { flexDirection: 'row', gap: 7, marginTop: 18 },
  serviceFilter: { flex: 1, minHeight: 38, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', borderRadius: 6 },
  serviceFilterSelected: { backgroundColor: colors.primary, borderColor: colors.primary },
  ...
  groupHeader: { minHeight: 62, borderWidth: 1, borderColor: colors.border, borderRadius: 8, backgroundColor: colors.surfaceMuted, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
