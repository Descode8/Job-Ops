...
export default function JobsScreen() {
  ...
  const loadJobs = useCallback(async () => {
    ...
    const { data: assignments } = await supabase.from('work_order_assignments').select('work_order:work_orders(id, work_order_number, title, status, deadline_at, properties(address_line_1, city, state))').eq('contractor_id', contractor.id).is('unassigned_at', null);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}><View><Text style={styles.kicker}>MARTY WRIGHT</Text><Text style={styles.title}>Assigned jobs</Text></View><TouchableOpacity style={styles.filterButton} accessibilityLabel="Filter jobs"><Ionicons name="options-outline" size={21} color={PAPER} /></TouchableOpacity></View>
        <Text style={styles.resultCount}>{isLoading ? 'LOADING...' : `${jobs.length} ASSIGNED JOB${jobs.length === 1 ? '' : 'S'}`}</Text>
        {!isLoading && jobs.length === 0 && <EmptyState message="No assigned work orders yet." />}
        {jobs.map((job) => <TouchableOpacity key={job.id} style={styles.jobCard} activeOpacity={0.85}><View style={styles.cardHeader}><View style={[styles.statusDot, { backgroundColor: job.status === 'in_progress' ? YELLOW : BLUE }]} /><Text style={styles.status}>{job.status.replace('_', ' ').toUpperCase()}</Text><Text style={styles.jobId}>#{job.work_order_number}</Text></View><Text style={styles.jobTitle}>{job.title}</Text><View style={styles.addressRow}><Ionicons name="location-outline" size={17} color={MUTED} /><Text style={styles.address}>{formatAddress(job.properties)}</Text></View><View style={styles.cardFooter}><Text style={styles.due}>{formatDeadline(job.deadline_at)}</Text><Ionicons name="arrow-forward" size={18} color={NAVY} /></View></TouchableOpacity>)}
      </ScrollView>
    </SafeAreaView>
  );
}
