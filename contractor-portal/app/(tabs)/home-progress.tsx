...
import { KeyboardAvoidingView, Modal, type NativeScrollEvent, type NativeSyntheticEvent, Platform, Pressable, RefreshControl, ScrollView, StyleSheet, TouchableOpacity, View } from 'react-native';
...
export default function HomeProgressScreen() {
  ...
  const ignoredRealtimeEventsRef = useRef(0);
  ...
  useWorkOrderRealtime(() => {
    if (ignoredRealtimeEventsRef.current > 0) {
      ignoredRealtimeEventsRef.current -= 1;
      return;
    }
    void load();
  });
  ...
  const toggleItem = async (home: Home, itemId: number) => {
    ...
    // The RPC also updates work_orders, which emits a realtime event. This
    // screen has already applied the checkbox change optimistically, so avoid
    // replacing the whole list (and disturbing its scroll position) afterward.
    ignoredRealtimeEventsRef.current += 1;
    setHomes((existing) => existing.map((entry) => entry.id === home.id ? { ...entry, work_order_checklist: nextRows } : entry));
    requestAnimationFrame(() => scrollViewRef.current?.scrollTo({ y: scrollOffsetRef.current, animated: false }));
    ...
    if (error) { ignoredRealtimeEventsRef.current = 0; Alert.alert('Could not update checklist', error.message); await load(); return; }
  };
  ...
  const completeHome = async (home: Home) => {
    ...
    const emailResult = await ensureCompletionEmail(home.id);
    setCompletingHomeId(null);
    setHomes((existing) => existing.filter((entry) => entry.id !== home.id));
    Alert.alert(!emailResult.ok ? 'Work order completed; email queued' : 'Work order completed', !emailResult.ok ? `${home.properties?.customer_name || home.title} was completed. Its email remains queued for retry.` : `${home.properties?.customer_name || home.title} was moved to completed work and a completion email was sent.`);
  };
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><ScrollView ref={scrollViewRef} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} onScroll={(event: NativeSyntheticEvent<NativeScrollEvent>) => { scrollOffsetRef.current = event.nativeEvent.contentOffset.y; }} scrollEventThrottle={16} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={onRefresh} tintColor={colors.primary} colors={[colors.primary]} />}>
