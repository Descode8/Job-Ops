...
export default function HomeProgressScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const [itemsResult, homesResult] = await Promise.all([
      ...
      supabase.from('work_orders').select('id, work_order_number, title, status, created_at, completed_at, properties(customer_name, address_line_1, city, state, postal_code), work_order_checklist(checklist_item_id, is_complete)').like('work_order_number', 'HOME-%').neq('status', 'completed').order('created_at', { ascending: false }),
    ]);
  ...
  const completeHome = async (home: Home) => {
    ...
    if (error) { Alert.alert('Could not complete work order', error.message); return; }
    setHomes((existing) => existing.filter((entry) => entry.id !== home.id));
    Alert.alert('Work order completed', `${home.properties?.customer_name || home.title} was moved to completed work.`);
  };
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <ImageBackground source={require('@/assets/images/dark-blue-particle-texture-background.jpg')} style={styles.header} contentFit="cover"><View><Text style={styles.kicker}>NEW HOME TRACKING</Text><Text style={styles.title}>Home Progress</Text></View><Ionicons name="clipboard" size={29} color={YELLOW} /></ImageBackground>
    ...
    {!isLoading && homes.length === 0 && <View style={styles.empty}><Ionicons name="home" size={34} color={colors.primary} /><Text style={styles.emptyTitle}>No homes added yet</Text><Text style={styles.emptyText}>Create a home to attach its name, address, and 14 checklist items.</Text></View>}
    {homes.map((home) => {
