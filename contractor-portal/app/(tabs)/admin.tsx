...
const NAVY = '#003366'; const BLUE = '#1E67B2'; const PAPER = '#FFFFFF'; const YELLOW = '#FFF200';
...
export default function AdminScreen() {
  ...
  const renderPersonCard = (item: ContractorView, isCurrent = false) => {
    ...
    <View key={item.contractor_id} style={styles.card}>
      <View style={styles.cardTop}>
        ...
        <View style={styles.person}><Text style={styles.name}>{item.full_name}</Text><Text style={styles.meta}>{item.email} · {item.phone_number}</Text></View>
        <View style={styles.cardActions}>
          ...
          <Text style={item.is_admin ? styles.adminBadge : styles.activeBadge}>{isCurrent ? 'ADMIN' : item.is_admin ? 'ADMIN' : 'ACTIVE'}</Text>
        </View>
      </View>
  ...
  const renderWorkOrder = (order: AdminWorkOrder) => (
    <TouchableOpacity key={order.id} style={styles.workOrderCard} activeOpacity={0.85} onPress={() => router.push({ pathname: '/work-order/[id]', params: { id: order.id } })}>
...
const createStyles = (colors: AppThemeColors) => StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, content: { flexGrow: 1, backgroundColor: colors.background, padding: 20, paddingBottom: 40 }, header: { backgroundColor: colors.header, marginHorizontal: -20, marginTop: -20, paddingHorizontal: 20, paddingTop: 22, paddingBottom: 25, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, kicker: { color: YELLOW, fontSize: 10, fontWeight: '900', letterSpacing: 1.4, marginBottom: 8 }, title: { color: PAPER, fontSize: 27, fontWeight: '800' }, form: { backgroundColor: colors.surface, marginHorizontal: -20, paddingHorizontal: 20, paddingTop: 21, paddingBottom: 25, marginBottom: 24 }, section: { color: colors.text, fontSize: 10, fontWeight: '900', letterSpacing: 1.1, marginBottom: 8 }, listSection: { color: colors.text, fontSize: 10, fontWeight: '900', letterSpacing: 1.1, marginBottom: 10 }, input: { minHeight: 47, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.input, borderRadius: 6, paddingHorizontal: 13, marginTop: 9, color: colors.text, fontSize: 13 }, createButton: { minHeight: 52, backgroundColor: '#2577BB', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 18, paddingHorizontal: 12, borderRadius: 6 }, createButtonPressed: { backgroundColor: '#1C1C5C' }, disabledButton: { opacity: 0.45 }, createText: { color: PAPER, fontSize: 12, fontWeight: '900', marginLeft: 8 }, help: { color: colors.textMuted, fontSize: 10, lineHeight: 15, marginTop: 9 }, credentials: { backgroundColor: colors.surfaceMuted, borderWidth: 1, borderColor: colors.accent, borderRadius: 6, padding: 13, marginTop: 14 }, credentialsTitle: { color: colors.text, fontSize: 9, fontWeight: '900', marginBottom: 8 }, credentialLabel: { color: colors.textMuted, fontSize: 7, fontWeight: '900', marginTop: 7 }, credentialValue: { color: colors.text, fontSize: 13, fontWeight: '900', marginTop: 2 }, credentialsHelp: { color: colors.textMuted, fontSize: 9, lineHeight: 14, marginTop: 10 }, dismissCredentials: { color: colors.primary, fontSize: 10, fontWeight: '900', textAlign: 'center', marginTop: 12, textDecorationLine: 'underline' }, card: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 7, padding: 15, marginBottom: 11 }, inactive: { opacity: 0.55 }, cardTop: { flexDirection: 'row', alignItems: 'flex-start' }, person: { flex: 1 }, cardActions: { alignItems: 'center', marginLeft: 10 }, name: { color: colors.text, fontSize: 15, fontWeight: '900' }, meta: { color: colors.textMuted, fontSize: 10, marginTop: 4 }, viewButton: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surfaceMuted, marginBottom: 7 }, adminBadge: { color: NAVY, backgroundColor: YELLOW, fontSize: 8, fontWeight: '900', padding: 6 }, activeBadge: { color: colors.primary, fontSize: 8, fontWeight: '900' }, job: { flexDirection: 'row', alignItems: 'center', borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 9, marginTop: 10 }, jobNumber: { color: colors.primary, fontSize: 9, fontWeight: '900' }, jobTitle: { flex: 1, color: colors.text, fontSize: 10, marginHorizontal: 8 }, jobStatus: { color: colors.textMuted, fontSize: 8, fontWeight: '900' }, noJobs: { color: colors.textMuted, fontSize: 10, marginTop: 12 }, removeButton: { borderTopWidth: 1, borderTopColor: colors.border, marginTop: 12, paddingTop: 11 }, removeText: { color: colors.danger, fontSize: 10, fontWeight: '900' }, denied: { flex: 1, alignItems: 'center', justifyContent: 'center' }, deniedTitle: { color: colors.text, fontSize: 16, fontWeight: '900', marginTop: 12 }, modalBackdrop: { flex: 1, backgroundColor: 'rgba(2, 8, 18, 0.72)', alignItems: 'center', justifyContent: 'center', padding: 20 }, contractorModal: { width: '100%', maxWidth: 470, backgroundColor: colors.surfaceElevated, borderWidth: 1, borderColor: colors.border, borderRadius: 18, padding: 20, shadowColor: '#000000', shadowOpacity: 0.28, shadowRadius: 18, shadowOffset: { width: 0, height: 10 }, elevation: 12 }, modalHeader: { flexDirection: 'row', alignItems: 'center' }, modalAvatar: { width: 52, height: 52, borderRadius: 26, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' }, modalAvatarImage: { width: 52, height: 52, borderRadius: 26, backgroundColor: 'transparent' }, modalAvatarText: { color: PAPER, fontSize: 17, fontWeight: '900' }, modalHeading: { flex: 1, marginLeft: 13 }, modalName: { color: colors.text, fontSize: 20, fontWeight: '900' }, modalRole: { color: colors.primary, fontSize: 9, fontWeight: '900', letterSpacing: 1, marginTop: 4 }, modalClose: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center' }, contactDetails: { marginTop: 22, borderTopWidth: 1, borderTopColor: colors.border }, contactRow: { minHeight: 65, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.border }, contactIcon: { width: 38, height: 38, borderRadius: 19, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' }, contactCopy: { flex: 1, marginLeft: 12 }, contactLabel: { color: colors.textMuted, fontSize: 8, fontWeight: '900', letterSpacing: 0.8 }, contactValue: { color: colors.text, fontSize: 13, fontWeight: '700', marginTop: 4 }, contactActions: { flexDirection: 'row', gap: 10, marginTop: 22 }, contactButton: { flex: 1, minHeight: 50, borderRadius: 8, backgroundColor: '#2577BB', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, contactButtonPressed: { backgroundColor: '#1C1C5C' }, contactButtonText: { color: PAPER, fontSize: 12, fontWeight: '900' }, groupLabel: { color: colors.textMuted, fontSize: 8, fontWeight: '900', letterSpacing: 1, marginBottom: 8 }, collapsibleHeader: { minHeight: 58, borderWidth: 1, borderColor: colors.border, borderRadius: 8, backgroundColor: colors.surfaceMuted, paddingHorizontal: 15, marginTop: 12, marginBottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, collapsibleTitle: { color: colors.text, fontSize: 13, fontWeight: '900', letterSpacing: 0.8 }, collapsibleCount: { color: colors.textMuted, fontSize: 8, fontWeight: '800', marginTop: 3 }, emptyGroup: { color: colors.textMuted, fontSize: 11, textAlign: 'center', paddingVertical: 18 }, assignmentCount: { color: colors.textMuted, fontSize: 10, marginTop: 12 }, workOrderCard: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 7, padding: 15, marginBottom: 10 }, workOrderTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, workOrderNumber: { color: colors.primary, fontSize: 10, fontWeight: '900' }, workOrderTitle: { color: colors.text, fontSize: 15, fontWeight: '900', marginTop: 11 }, workOrderAddress: { color: colors.textMuted, fontSize: 10, marginTop: 6 }, listAvatar: { width: 46, height: 46, borderRadius: 23, marginRight: 12 }, listAvatarFallback: { width: 46, height: 46, borderRadius: 23, marginRight: 12, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' }, listAvatarText: { color: colors.primary, fontSize: 13, fontWeight: '900' } });

const createAdminDetailStyles = (colors: AppThemeColors) => StyleSheet.create({
  assignmentToggle: { minHeight: 42, borderTopWidth: 1, borderTopColor: colors.border, marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  assignmentList: { borderWidth: 1, borderColor: colors.border, borderRadius: 7, overflow: 'hidden', marginBottom: 4 },
  assignmentRow: { minHeight: 62, paddingHorizontal: 11, paddingVertical: 9, borderTopWidth: 1, borderTopColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 9, backgroundColor: colors.surfaceMuted },
  ...
  workOrderModal: { width: '100%', maxWidth: 560, maxHeight: '86%', backgroundColor: colors.surfaceElevated, borderWidth: 1, borderColor: colors.border, borderRadius: 16, overflow: 'hidden' },
  modalTop: { minHeight: 78, paddingLeft: 18, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.border },
  modalHeading: { flex: 1 },
  ...
  viewFullButton: { minHeight: 52, backgroundColor: '#2577BB', borderRadius: 7, marginTop: 23, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  viewFullPressed: { backgroundColor: '#1C1C5C' },
