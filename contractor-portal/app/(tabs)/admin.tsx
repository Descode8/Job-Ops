...
export default function AdminScreen() {
  ...
  const renderUnacceptedWorkOrder = ({ offer, order, contractor }: (typeof unacceptedWorkOrders)[number]) => (
    <TouchableOpacity key={offer.id} style={[styles.workOrderCard, { borderLeftWidth: 3, borderLeftColor: workOrderPriorityColor(order.priority) }]} activeOpacity={0.85} onPress={() => router.push({ pathname: '/work-order/[id]', params: { id: order.id } })}>
      <View style={styles.workOrderTop}><Text style={styles.workOrderNumber}>{formatWorkOrderNumber(order.work_order_number)}</Text><Text style={styles.awaitingBadge}>AWAITING ACCEPTANCE</Text></View>
      <Text style={styles.workOrderTitle}>{order.properties?.customer_name || order.title}</Text>
      <Text style={styles.workOrderAddress}>{formatAddress(order.properties)}</Text>
      <Text style={styles.assignedContractor}>ASSIGNED TO {contractor?.full_name?.toUpperCase() || 'UNKNOWN CONTRACTOR'}</Text>
      <Text style={styles.workOrderDeadline}>{formatWorkOrderDeadline(order.deadline_at)}</Text>
    </TouchableOpacity>
  );
