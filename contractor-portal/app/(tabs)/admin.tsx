...
type PendingWorkOrderOffer = { id: string; work_order_id: string; recipient_id: string; created_at: string };
...
export default function AdminScreen() {
  ...
  const [pendingOffers, setPendingOffers] = useState<PendingWorkOrderOffer[]>([]);
  ...
  const [unacceptedExpanded, setUnacceptedExpanded] = useState(true);
  ...
  const load = useCallback(async () => {
    ...
    const [{ data, error }, { data: orderRows, error: orderError }, { data: offerRows, error: offerError }, { data: avatarRows }] = await Promise.all([
      ...
      supabase.from('work_order_offers').select('id, work_order_id, recipient_id, created_at').eq('status', 'pending').order('created_at', { ascending: false }),
    ...
    if (offerError) { Alert.alert('Could not load unaccepted work orders', offerError.message); return; }
    ...
    setPendingOffers((offerRows ?? []) as PendingWorkOrderOffer[]);
  ...
  const createContractor = async () => {
    ...
    Alert.alert(
      ...
      data.smsSent
        ? 'The contractor received a text with their username and temporary password. The login details are also shown on the Admin screen.'
        : `${data.smsError ?? 'The invitation text could not be sent.'} Give the contractor the temporary login details shown on the Admin screen.`,
    );
    await load();
  };
  ...
  const unacceptedWorkOrders = pendingOffers.flatMap((offer) => {
    const order = workOrders.find((item) => item.id === offer.work_order_id);
    const contractor = contractors.find((item) => item.contractor_id === offer.recipient_id);
    return order ? [{ offer, order, contractor }] : [];
  });
  ...
  const renderUnacceptedWorkOrder = ({ offer, order, contractor }: (typeof unacceptedWorkOrders)[number]) => (
    <TouchableOpacity key={offer.id} style={[styles.workOrderCard, { borderLeftWidth: 3, borderLeftColor: workOrderPriorityColor(order.priority) }]} activeOpacity={0.85} onPress={() => router.push({ pathname: '/work-order/[id]', params: { id: order.id } })}>
      <View style={styles.workOrderTop}><Text style={styles.workOrderNumber}>{formatWorkOrderNumber(order.work_order_number)}</Text><View style={styles.awaitingStatus}><Text style={styles.awaitingBadge}>AWAITING ACCEPTANCE</Text><Text style={styles.awaitingContractor}>{contractor?.full_name?.toUpperCase() || 'UNKNOWN CONTRACTOR'}</Text></View></View>
      <Text style={styles.workOrderTitle}>{order.properties?.customer_name || order.title}</Text>
      <Text style={styles.workOrderAddress}>{formatAddress(order.properties)}</Text>
      <Text style={styles.workOrderDeadline}>{formatWorkOrderDeadline(order.deadline_at)}</Text>
    </TouchableOpacity>
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><View style={styles.header}><View><Text style={styles.kicker}>JOBOPS · ADMIN</Text><Text style={[styles.title, { fontWeight: '900' }]}>Contractor Management</Text></View><Ionicons name="shield-checkmark" size={28} color={themeMode === 'black' ? PAPER : YELLOW} /></View>
    ...
    <CollapsibleHeader label="UNACCEPTED WORK ORDERS" count={unacceptedWorkOrders.length} expanded={unacceptedExpanded} onPress={() => setUnacceptedExpanded((value) => !value)} />
    {unacceptedExpanded && (unacceptedWorkOrders.length ? unacceptedWorkOrders.map(renderUnacceptedWorkOrder) : <Text style={styles.emptyGroup}>No work orders are awaiting acceptance.</Text>)}
