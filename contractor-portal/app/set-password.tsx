...
const PAPER = '#FFFFFF'; const YELLOW = '#FFF200';
...
export default function SetPasswordScreen() {
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.header}><View style={styles.toggle}><ThemeToggle /></View><Ionicons name="key-outline" size={30} color={YELLOW} /><Text style={styles.title}>Create Password</Text></View><View style={styles.form}><Text style={styles.help}>{ready ? 'Choose a permanent password for your contractor account.' : 'Your login session could not be verified. Return to login and enter your temporary password again.'}</Text><TextInput style={styles.input} value={password} onChangeText={setPassword} placeholder="New password" placeholderTextColor={colors.textMuted} secureTextEntry autoCapitalize="none" autoCorrect={false} /><View style={styles.rules}>{rules.map((rule) => <View key={rule.label} style={styles.rule}><Ionicons name={rule.met ? 'checkmark-circle' : 'ellipse-outline'} size={17} color={rule.met ? colors.success : colors.textMuted} /><Text style={[styles.ruleText, rule.met && styles.ruleMet]}>{rule.label}</Text></View>)}</View><TextInput style={styles.input} value={confirm} onChangeText={setConfirm} placeholder="Confirm password" placeholderTextColor={colors.textMuted} secureTextEntry autoCapitalize="none" autoCorrect={false} />{confirm.length > 0 && <View style={styles.matchRow}><Ionicons name={passwordsMatch ? 'checkmark-circle' : 'close-circle'} size={17} color={passwordsMatch ? colors.success : colors.danger} /><Text style={[styles.matchText, { color: passwordsMatch ? colors.success : colors.danger }]}>{passwordsMatch ? 'Passwords match' : 'Passwords do not match'}</Text></View>}<Pressable style={({ pressed }) => [styles.button, pressed && canSubmit && styles.buttonPressed, !canSubmit && styles.disabled]} disabled={!canSubmit} onPress={() => void save()}><Text style={styles.buttonText}>{saving ? 'Saving password...' : 'Create Password'}</Text></Pressable></View></SafeAreaView>;
}
const createStyles = (colors: AppThemeColors) => StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, header: { minHeight: 150, backgroundColor: colors.header, padding: 24, justifyContent: 'flex-end' }, toggle: { position: 'absolute', right: 20, top: 18 }, title: { color: PAPER, fontSize: 27, fontWeight: '900', marginTop: 12 }, form: { padding: 22 }, help: { color: colors.textMuted, fontSize: 13, lineHeight: 20, marginBottom: 14 }, input: { minHeight: 50, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.input, borderRadius: 6, paddingHorizontal: 13, marginTop: 12, color: colors.text }, rules: { backgroundColor: colors.surfaceMuted, borderRadius: 6, padding: 12, marginTop: 10 }, rule: { flexDirection: 'row', alignItems: 'center', marginVertical: 4 }, ruleText: { color: colors.textMuted, fontSize: 11, marginLeft: 8 }, ruleMet: { color: colors.success, fontWeight: '800' }, matchRow: { flexDirection: 'row', alignItems: 'center', marginTop: 9 }, matchText: { fontSize: 11, fontWeight: '800', marginLeft: 7 }, button: { minHeight: 52, backgroundColor: colors.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 18, paddingHorizontal: 12, borderRadius: 6 }, buttonPressed: { backgroundColor: colors.primaryStrong }, disabled: { opacity: 0.45 }, buttonText: { color: PAPER, fontSize: 12, fontWeight: '900', marginLeft: 8 } });
